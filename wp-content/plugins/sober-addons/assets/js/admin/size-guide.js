jQuery(document).ready(function ($) {
	...
	// Render tabs.
	var $tabs = $( '#sober-size-guide-tabs' ),
		$tabsNav = $( '.sober-size-guide-tabs--tabs', $tabs ),
		$addTab = $( '.add-new-tab', $tabsNav ),
		templates = {
			tab: wp.template( 'sober-size-guide-tab' ),
			panel: wp.template( 'sober-size-guide-modal' )
		};

	if ( ! _.isUndefined( soberSizeGuideTables ) ) {
		for ( var i = 0; i < soberSizeGuideTables.tables.length; i++ ) {
			$addTab.before( templates.tab( {
				index: i,
				tab: soberSizeGuideTables.tabs[i]
			} ) );

			$tabs.append( templates.panel( {
				index: i,
				name: soberSizeGuideTables.names[i],
				description: soberSizeGuideTables.descriptions[i],
				table: soberSizeGuideTables.tables[i],
				information: soberSizeGuideTables.information[i]
			} ) );

			$tabs.find( '.sober-size-guide-table-editor[data-tab='+ i +']' ).find( '.sober-size-guide-table' ).soberEditTable();

			$addTab.data( 'index', i );

			if ( 0 === i ) {
				$tabsNav.children( ':eq(0)' ).addClass( 'active' );
				$tabs.find( '.sober-size-guide-table-editor[data-tab='+ i +']' ).addClass( 'active' );
			}
	...
	// Change active tab.
	$tabsNav.on( 'click', '.sober-size-guide-table-tabs--tab', function() {
		var $el = $( this );
		...
		$tabs.find( '.sober-size-guide-table-editor[data-tab='+ $el.data( 'tab' ) +']' ).addClass( 'active' ).siblings( '.active' ).removeClass( 'active' );
	} );

	// Edit tab title.
	$tabsNav
		.on( 'click', '.edit-button', function() {
			$( this ).closest( '.sober-size-guide-table-tabs--tab' ).addClass( 'editting' ).children( 'input' ).removeClass( 'hidden' );
		} )
		.on( 'click', '.confirm-button', function() {
			$( this ).closest( '.sober-size-guide-table-tabs--tab' ).removeClass( 'editting' ).children( 'input' ).addClass( 'hidden' );
		} )
		.on( 'input', 'input', function() {
			$( this ).siblings( '.sober-size-guide-table-tabs--tab-text' ).text( this.value );
		} );
	...
	$tabsNav.on( 'click', '.add-new-tab', function() {
		...
		$tabs.find( '.sober-size-guide-table-editor[data-tab='+ index +']' ).find( '.sober-size-guide-table' ).soberEditTable();
	...
	$tabs.on( 'click', '.delete-table', function( event ) {
		...
		var $deletedPanel = $( this ).closest( '.sober-size-guide-table-editor' ),
	...
	$tabs.on( 'tab_added tab_removed', function() {
		if ( $tabs.children( '.sober-size-guide-table-editor' ).length <= 1 ) {
...
(function ($, window, i) {
	...
	$.fn.soberEditTable = function (options) {

		// Settings
		var s = $.extend({
				data: [ ['']],
				tableClass: 'sober-table-edit',
				jsonData: false,
				headerCols: false,
				maxRows: 999,
				first_row: true,
